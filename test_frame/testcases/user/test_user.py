...
class TestUser:
    ...
    @allure.story(next(c_id)+'网易新闻接口')
    @allure.description("描述：网易新闻接口")
    @pytest.mark.run(order=4)
    @allure.link(url="https://api.apiopen.top/getWangYiNews", name="接口地址")
    @allure.severity(allure.severity_level.BLOCKER)
    @pytest.mark.parametrize("caseinfo", read_testcase_file('/testcases/user/wy_new.yaml'))
    def test_wy_new(self, caseinfo):
        allure.dynamic.title(caseinfo['name'])
        allure.dynamic.description(caseinfo['name'])
        RequestUtil().analysis_yaml(caseinfo)