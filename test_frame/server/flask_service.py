...
def user_login():
    ...
    :return:
    """
    user_info = {'user_name': 'test', 'passwd': '123456'}
    ...
    if all([user_name, passwd]):
        if all([user_name == user_info['user_name'], passwd == user_info['passwd']]):
            return jsonify({'msg': '登录成功！', 'msg_code': 200, 'error_code': 0, 'token': token})
        else:
            return jsonify({'msg': '登录失败，用户名或密码错误！', 'msg_code': 9001, 'token': -1})
    else:
...
def add_user():
    """新增用户接口"""
    add_user_info = {'username': 'adduser',
                     'password': '123456',
                     'role_id': '123456789',
                     'dates': '2023-12-31',
                     'phone': '13800000000',
    ...
    if username == add_user_info['username'] and password == add_user_info['password'] and role_id == add_user_info[
        ...
        return jsonify({'msg': '新增成功！', 'msg_code': 200, 'error_code': 0})
    else:
...
def delete_user():
    ...
    if user_id in user_id_lst:
        return jsonify({'msg': '删除成功！', 'msg_code': 200, 'error_code': 0})
    else:
...
def update_user():
    ...
    if username == update_user_info['username'] and password == update_user_info['password'] and role_id == \
            update_user_info['role_id'] and dates == update_user_info['dates'] and phone == update_user_info['phone']:
        return jsonify({'msg': '更新成功！', 'msg_code': 200, 'error_code': 0})
    else:
...
def query_user():
    ...
    if user_id in query_id_lst:
        return jsonify({'msg': '查询成功！', 'msg_code': 200, 'error_code': 0})
    else:
...
pro_info = {'goodsList': [{'goodsId': '18382788819',
...
def add_cart():
    ...
    if all([goods_id, count, price]):
        if goods_id in good_id_list:
            response = {"createTime": now_date(),
                        "error": "",
                        "error_code": "0000",
                        "message": "success",
                        "translate_language": "zh-CN",
                        "userId": user_id,
                        "cartList": [{"cid": random.sample(range(0, 1000), 1)[0],
            ...
            return jsonify({"createTime": now_date(),
                            "error": "商品id不存在！",
                            "error_code": "4000",
                            "message": "",
                            "translate_language": "zh-CN",
...
def place_an_order():
    """商品下单接口--提交订单"""
    place_info = {"goods_id": '33809635011',
                  "number": 3,
    ...
    if all([goods_id, number, propertyChildIds, inviter_id, price, freight_insurance, discount_code, consignee_info]):
        if goods_id in good_id_list:
            ...
            write_data(DIR_BASE + '/data/mockdata/orderNumber.json', json.dumps({'order_num': order_num, 'user_id': user_id}))
            response = {'orderNumber': order_num,
                        'userId': user_id,
                        'crateTime': now_date(),
                        'error': '',
                        'error_code': '0000',
...
if __name__ == '__main__':
    # 第一种启动模式
    # debug=True，开启debug模式，改代码后不用重启，会自动重启
